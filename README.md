...
5. journey 이중계상 방지: M1 외래/triage 수익 ↔ RCM(Part A DRG)은 별개 stream. M3(APCM/RPM)와 W(AWV/스크리닝)는 월반복·연1회 관문으로 중복 청구하지 않는다.
...
## 4.2 5개년 동적 모델 — v3.7.2 핵심

v3.7.2는 v3.7.1의 정적 Y1 EBITDA 엔진을 유지하되, 병원 확산에 따라 입력값이 매년 변하는 5개년 뷰를 추가한다. 핵심은 "같은 레버라도 볼륨이 늘면 M1/M2/M3/W, LOS, VBC 모수가 함께 커진다"는 점이다.
...
## 5. B1 Patient Journey Engine — v3.8 핵심

v3.8부터 W/M1/M2/M3 자체를 인바이츠 루프 patient journey에 정합시킨다. 원격케어는 단일 PMPM이 아니라 M1 외래 진입에서 W wellness 복귀까지 이어지는 여정 전체의 capture engine이다.

```text
M1 외래 진입·triage = 내원/접수/진료 + IRS+ 생성 + 앱 activation
M2 입원·처치·회복   = 입원/수술/처치/병상 회복 + 퇴원 전이 준비
M3 퇴원후 회복·Loop = 통원·원격케어·APCM/RPM 월 반복 관리
W  wellness 복귀    = 회복 후 AWV·스크리닝·재발방지·APCM 재진입 관문
```

계산식:

```text
M1 외래/triage  = panel × M1포착률 × $400 × margin
M2 acute care   = 퇴원수 × TCM적격률 × $230 × margin
M3 Loop care    = 등록panel(panel×π) × netPMPM × 12
W  wellness     = panel × AWV포착률 × $150 × margin
```
...
## 5.1 IRS+ M1 activation ROI — v3.8

IRS+는 자체 환급 수익을 0으로 둔다. 대신 M1 외래 triage 단계에서 병원이 부담하는 activation 투자비로 처리하고, 이후 M2/M3의 포착률·유지율 개선으로 회수 가능한지 본다.

```text
IRS+ 비용 = discharges × IRS+대상비중 × IRS+생성/가입률 × IRS+ 1인당 원가

IRS+ 효과:
effPi      = pi + IRS+ π uplift
effAdher   = adher + IRS+ 유지율 uplift
effM1Rate  = m1Rate + IRS+ M1 포착률 uplift
effTcmRate = tcmRate + IRS+ TCM 포착률 uplift
```

기본값은 IRS+ 대상 35%, 생성/가입률 70%, 1인당 원가 $180, π +6pp, TCM +8pp, M1 +5pp, 유지율 +5pp다. 이 값은 실증 전 가설 입력이며, Go/No-go 판단에서는 보수·기본·공격 시나리오로 민감도 검토한다.
...
- `COHORTS` + `AWV_VAL`·`M1_VAL`·`TCM_VAL` — patient journey 단위값. 스택·단가 수정 여기.
- IRS+ 입력(`irsEligibleRate`·`irsActivationRate`·`irsCostPer`·uplift pp) — M1 activation ROI 가정. 자체 환급은 0으로 두고 포착률·유지율 uplift만 반영.
...
- [ ] panel을 올리면 M1·M3·W가 커지고, 퇴원수를 올리면 M2·LOS·VBC가 커지나?
- [ ] IRS+ ON/OFF 시 자체 수익은 0이고 비용은 opex에 들어가며, M1/M2/M3 uplift만 반영되나?
...
| payer-gated | payer별로 회수 가능성이 다르게 제한된다는 뜻. 같은 gross라도 TM, MA, Medicaid, Commercial별 EBITDA 반영률이 다르다. |
| Medicare | 미국 연방 노인·장애인 의료보험. 이 모델에서는 TM과 MA로 나눈다. |
| CMS | Centers for Medicare & Medicaid Services. Medicare/Medicaid 정책·지불제도·품질지표를 운영하는 미국 정부기관. |
...
| FFS | Fee-for-Service. 서비스 제공량마다 건별로 지급받는 방식. |
| ACO | Accountable Care Organization. 총비용을 줄이면 shared savings를 받을 수 있는 책임진료 구조. |
| REACH | ACO REACH 모델. Medicare 책임진료·위험분담 모델 중 하나로, APCM/VBC 요건 설명에 등장한다. |
| MIPS MVP | Merit-based Incentive Payment System Value Pathways. Medicare 품질·성과 보고 체계의 한 유형. |
...
| capture | 만든 가치나 절감액 중 병원 EBITDA로 실제 회수되는 몫. |
...
| HCC | Hierarchical Condition Category. Medicare Advantage 등에서 환자 중증도·위험도를 반영하는 위험조정 체계. |
...
| audit exposure | 코딩·청구가 outlier일 때 감사나 삭감 위험이 커지는 상태. PEPPER 80p 초과 경고와 연결된다. |

### 14.4 Patient Journey Engine

| 용어 | 의미 |
| --- | --- |
| Patient Journey Engine M1→M2→M3→W | 환자가 외래 진입·triage, 입원·처치·회복, 퇴원후 Loop 관리, wellness 복귀로 이동하는 여정을 EBITDA 관점으로 계산하는 엔진. |
| M1 | 외래 진입·triage 단계. 내원/접수/진료, IRS+ 생성, 앱 activation을 포함한다. 계산상 `panel × M1포착률 × M1_VAL × margin`이다. |
| M2 | 입원·처치·회복 단계. 입원/수술/처치/병상 회복과 퇴원 전이 준비를 포함한다. 계산상 TCM 포착률과 퇴원수에 연동된다. |
| M3 | 퇴원후 Loop 회복관리 단계. 통원·원격케어·APCM/RPM/BHI/CoCM 월 반복 관리가 쌓이는 stock 영역이다. |
| W | wellness 복귀·재발방지 단계. AWV·스크리닝·APCM 재진입 관문을 의미한다. |
| IRS+ | M1 triage에서 생성하는 개인 위험도/건강위험 스코어. 자체 환급은 0으로 두고, M2/M3 포착률·유지율 uplift를 통해 ROI를 본다. |
| AWV | Annual Wellness Visit. Medicare 연차 건강검진. |
| 스크리닝 | 환자 상태·위험을 확인하는 검사/평가 활동. |
| Part A | Medicare 입원·시설성 급여 영역. DRG 입원지불액과 연결된다. |
| Part B | Medicare 의사·전문의료·외래성 급여 영역. v3.8에서는 M1 외래/triage 포착액의 일부 성격으로 본다. |
...
| CCM | Chronic Care Management. 만성질환 관리 청구. APCM과 배타 관계가 있어 중복 청구를 조심해야 한다. |
| PCM | Principal Care Management. 특정 주요 질환 중심 관리 청구. APCM/CCM 등과 배타 관계를 확인해야 한다. |
...
| flow | 해당 연도에 발생한 퇴원수·방문수 같은 흐름값. v3.8에서는 M1/W는 panel flow, M2는 퇴원수 flow, M3는 등록 panel stock 성격이다. |
...
| weightedGross | 코호트별 월 gross 청구액을 mix로 가중평균한 값. M3 PMPM 계산에 쓴다. |
...
| 화면 입력 | 의미 |
| --- | --- |
| 전체 퇴원수/년 / discharges | 1년 동안 병원에서 퇴원한 입원 건수. M2, LOS, VBC, IRS+ 대상자 계산의 주요 모수다. |
...
| 목표 실현율 / rcmTarget | RCM 개선 후 도달한다고 보는 목표 실현율. 보수/기본/공격 시나리오별로 다르다. |
...
| PEPPER cap | PEPPER outlier 위험이 높을 때 RCM 회수 가능액을 낮추는 상한. 화면에서는 80p 초과 시 50% 감액한다. |
...
| 사칙연산 | 더하기, 빼기, 곱하기, 나누기. 계산식 편집 패널에서 허용되는 기본 연산이다. |
...
| S-curve | 낮은 구간에서는 천천히, 임계점을 넘으면 빠르게, 상단에서는 다시 완만해지는 곡선. κ 계산에 사용한다. |
| inflection | S-curve가 빠르게 올라가기 시작하는 전환점. 화면에서는 π 30%를 임계점으로 둔다. |
| selBias | 선택편의 보정계수. 건강관리 의지나 기관 선택효과로 업사이드가 과대평가되는 것을 낮춘다. |
| savEp | episode당 CMS 절감액 가정. CMS AHCAH 실측 밴드를 기반으로 한다. |
| AHCAH | Acute Hospital Care at Home. CMS 병원-재택치료 프로그램 데이터로, savEp 밴드의 근거로 사용했다. |
| dLOS | LOS 단축 일수 가정. |
| losReal | LOS 단축분 중 실제 비용절감으로 실현되는 비율. |
| cmCase | 추가 입원/볼륨 1건당 기여 margin 가정. |
| hccVal | MA HCC 위험조정 또는 품질 업사이드의 환자당 가치 가정. |
| sSav | 원격케어 청구가 ACO shared savings를 일부 줄이는 음결합 계수. |
...
| tm / ma / medicaid / commercial | payer mix를 0~1 비율로 바꾼 계산 변수. 예: TM 35%는 `tm=0.35`. |
| aco | ACO 귀속 TM 비율을 0~1로 바꾼 계산 변수. |
| partA / selfPay / agency | 화면 입력의 `$M` 값을 달러 단위로 바꾼 계산 변수. |
| hrrBasePartA | HRR 계산에 쓰는 기준 Part A 금액. v3.8 동적 계산에서는 baseline을 유지해 과대계상을 막는다. |
| losEligible | LOS 절감이 적용되는 payer 비중. v3.8 기본은 `tm + medicaid`. |
| teleGrossAll | M1+M2+M3+W의 payer 적용 전 journey gross 합계. |
| teleTM / teleMA / teleMedicaid / teleCommercial | payer별 원격케어 회수액. |
| telecare | payer별 원격케어 회수액을 합친 값. |
| clawback | ACO 음결합 차감액. |
| w / m1 / m2 / m3 | 계산식 안에서 쓰는 W, M1, M2, M3 patient journey별 금액 변수. |
| enrolled | `panel × pi`로 계산된 등록 환자 수. |
| selfpayConv | Self-Pay 전환 레버 금액. |
| c_selfpay | Self-Pay 원가회피 금액. |
| addrDisch | LOS 단축 대상 퇴원수. `discharges × losEligible × losAddr`로 계산한다. |
| c_los | LOS 단축에 따른 변동비 절감액. |
| c_hrr | HRR/HRRP 페널티 회피액. |
| c_agency | Agency Labor 절감액. |
| cmsSavingsPool | TM ACO 귀속 퇴원에서 발생 가능한 CMS 절감 풀. |
| b3_tm | TM shared savings 중 병원 회수분. |
| b3_ma | MA HCC·품질 업사이드 중 병원 회수분. |
| b1 / b2 / b3 / b4 | 각 레이어의 계산 결과 변수. |
| b0Local / b0Panel | B0를 local recapture와 panel 활성화로 나눈 계산 변수. |
...
| throughput | LOS 단축으로 빈 병상이 생겼을 때 추가 환자를 받을 수 있어 생기는 기여 margin. |
| freedBedDays | LOS 단축으로 비는 병상일수. |
| addlDischarges | freedBedDays를 평균 LOS로 나누어 계산한 추가 가능 퇴원수. |
...
- v3.3 legacy B1 경로 청구 엔진(W·M1·M2 flow=퇴원수 / M3 stock=panel) · M1↔RCM 경계 분리
...
- v3.7.2 5개년 동적 확산 · B0 Volume Expansion · B4 RHTP · RCM 실현율 갭 · 내부 수식 편집 범위 확대
- v3.8 Patient Journey 정합(M1 외래 triage→M2 입원·처치→M3 퇴원후 Loop→W wellness) · IRS+ M1 activation ROI 레버
